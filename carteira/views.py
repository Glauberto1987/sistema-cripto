...
def dashboard(request):
    ...
    if HistoricoPatrimonio.objects.count() <= 1:
        ...
        for t in transacoes_ord:
            ...
            data_t = t.data.date() if hasattr(t.data, 'date') else t.data
    ...
    tooltips_historico_list = [] # 🚨 Lista para as etiquetas de passar o mouse 🚨
    ...
    for h in historico:
        ...
        # Verifica quais moedas foram compradas/vendidas nesse exato dia
        dia_str = h.data.strftime('%d/%m/%Y')
        detalhes_dia = []
        
        for t in todas_transacoes:
            t_data_str = t.data.strftime('%d/%m/%Y')
            if t_data_str == dia_str:
                if t.tipo_operacao == 'COMPRA':
                    detalhes_dia.append(f"🟢 Comprou {t.moeda.simbolo}")
                else:
                    detalhes_dia.append(f"🔴 Vendeu {t.moeda.simbolo}")
        
        # Junta tudo para mandar para o gráfico
        if detalhes_dia:
            tooltips_historico_list.append(" | ".join(detalhes_dia))
        elif h.data == hoje:
            tooltips_historico_list.append("💰 Valor Atual Flutuante")
        else:
            tooltips_historico_list.append("Nenhuma transação")
    ...
    contexto = {
        ...
        'tooltips_historico': tooltips_historico, # Enviando a etiqueta pro HTML
    }
